...
import { ISignUpData, ISignUpState } from "../../../interfaces/signup.interface";
  ...
  export function validateForm(setErrors: Dispatch<SetStateAction<Partial<ISignUpData>>>, signUpState: ISignUpState) {
    ...
    if (!signUpState.signUpData.password || signUpState.signUpData.password.length < 8) {
      newErrors.password = 'Password should have at least 8 characters';
    } else if (!/(?=.*\d)(?=.*[a-z])(?=.*[A-Z])/.test(signUpState.signUpData.password)) {
    ...
    if (!signUpState.signUpData.shippingCity && !signUpState.signUpData.sameAddress) {
